...
class Tasks extends CI_Model {
	...
	/*
	 *  ПОЛУЧЕМ ВСЕ ИНСТРУКЦИИ С ЗАДАНЫМ ПОИСКОМ И СТРАНИЦЕЙ
	 */
	function getAllIns($searchTerm = false , $page = 1, $numberPerPage = false){
		$out = array();
		// search
		if($searchTerm !== false && strlen(trim($searchTerm)) > 1){
			$query = $this->db->like('multiscraper_ins.name', $searchTerm);
			$query = $this->db->or_like('multiscraper_ins.comment', $searchTerm);
		}
		// pager
		if($numberPerPage !== false && (int) $numberPerPage > 0 && (int) $page > 0){
			//echo 'numberPerPage:' . $numberPerPage . '<br />';
			//echo 'page:' . $page . '<br />';
			$this->db->limit($numberPerPage, ($numberPerPage*($page - 1)));
		}
		// GET INS
	...
	function getInsCount($searchTerm = false){
		if($searchTerm !== false && strlen(trim($searchTerm)) > 1){
			$this->db->like('name', $searchTerm);
			$this->db->or_like('comment', $searchTerm);
			$query = $this->db->get('multiscraper_ins');
			return $query->num_rows();
		}else{
			return $this->db->count_all('multiscraper_ins');
		}
	}
	...
	function joinInsProducts($ins_id = false){
		...
		if($query !== false){
			...
			$this->load->dbutil();
			if (!$this->dbutil->optimize_table('multiscraper_tasks')){
				$this->dbutil->repair_table('multiscraper_tasks');
			}
		}
		return $out;
	}
	...
	function createIns($data){
		...
		$donor_currency = isset($data['donor_currency'])?((int) $data['donor_currency'] > 0?$data['donor_currency']:0):0;
		$data['donor_currency'] = $donor_currency;
		...
		$other_data = array('get_options' => $data['get_options'],
			...
			'do_not_update_attributes' => $data['do_not_update_attributes'],
			'tax_class_id' => isset($data['tax_class_id'])?$data['tax_class_id']:0,
			'do_not_update_manufacturer' => isset($data['do_not_update_manufacturer'])?$data['do_not_update_manufacturer']:'',
			'do_not_update_taxclass' => isset($data['do_not_update_taxclass'])?$data['do_not_update_taxclass']:'',
		...
		unset($data['get_attributes']);
		unset($data['do_not_update_attributes']);
	...
	function updateIns($id , $data){
		...
		$other_data = array('get_options' => $data['get_options'],
							...
							'get_attributes' => $data['get_attributes'],
							'do_not_update_attributes' => $data['do_not_update_attributes'],
							'tax_class_id' => isset($data['tax_class_id'])?$data['tax_class_id']:'',
							'do_not_update_manufacturer' => isset($data['do_not_update_manufacturer'])?$data['do_not_update_manufacturer']:'',
							'do_not_update_taxclass' => isset($data['do_not_update_taxclass'])?$data['do_not_update_taxclass']:'',
							'vendor' => isset($data['vendor'])?$data['vendor']:0,
							'main_image_limit' => $data['main_image_limit'],
	 ...
	 *  DELETE INS  
	 */
	function deleteIns($ids , $with_products = false){
		$delete_products_ids = array();
		if(is_array($ids) && count($ids) > 0){
			//echo '<pre>'.print_r($ids , 1).'</pre>';exit;
			foreach($ids as $id){
	...
	function setPriority($ins_ids , $priority){
		//echo '<pre>'.print_r($ins_ids , 1).'</pre>';exit;
		$this->db->where_in("id" , $ins_ids);
		$this->db->update('multiscraper_ins', array('priority' => $priority));
		return true;
	}
	
	function setSwitch($ins_ids , $switch){
		$this->db->where_in("id" , $ins_ids);
	...
	function getGrabbedProducts($target , $start = 0 , $length = 10 , $search = false , $sort_cell = false, $sort_direction = false){
		$out = array();	
		$data = array();
		$recordsTotal = 0;
		$recordsFiltered = 0;
		...
		if($sort_cell !== false && $sort_direction !== false){
			$this->db->order_by($sort_cell , strtoupper($sort_direction));
		}else{
		...
		// echo '<pre>OUTN OF getGrabbedProducts FUNCTION : '.print_r($out , 1) .'</pre>';// exit;
		return $out;
	}
	 ...
	 */
	function getInstructionsIdsWithGrabbedProducts($searchTerm = false){
		$out = array();
		...
		if(strlen(trim($url)) > 0){
			...
			if($task_exists){
				...
				$this->db->insert('multiscraper_tasks', array(
															...
															'busy' => 0,
															'parent_id' => $parent_id,
															'p_date' => "1970-01-01 00:00:00",
															'p_date_update' => "1970-01-01 00:00:00"
															));
	...
	function getTasksForProcess($num){
		...
		$this->db->where('multiscraper_tasks.product_id != ' , '1');
		// $this->db->where('multiscraper_tasks.parsed != ' , '1');
