...
function mspro_sammydress_title($html){
    $res =  explode('<p class="titleMask">' , $html);
    if(count($res) > 1){
        $res = explode('</' , $res[1]);
        if(count($res) > 1){
        ...
        $instruction = 'div.pro_m_inner h1';
...
function mspro_sammydress_options($html){
	...
	if (is_array($res) && count($res) > 0){
	    foreach($res as $pos_option){
	        ...
	        }elseif(isset($pos_option['div']['strong'][0]['#text']) && 
	           !is_array($pos_option['div']['strong'][0]['#text']) && 
	           strlen(trim($pos_option['div']['strong'][0]['#text'])) > 0 && 
	           isset($pos_option['div']['ul'][0]['li']) && 
	           is_array($pos_option['div']['ul'][0]['li']) && 
	           count($pos_option['div']['ul'][0]['li']) > 0){
    	            $OPTION = array();
    	            $OPTION['name'] = str_ireplace(array(":") , array("") , trim($pos_option['div']['strong'][0]['#text']) );
    	            $OPTION['type'] = "select";
    	            $OPTION['required'] = true;
    	            $OPTION['values'] = array();
    	            foreach($pos_option['div']['ul'][0]['li'] as $option_value){
    	                if(isset($option_value['a'][0]['title']) && !is_array($option_value['a'][0]['title']) && strlen(trim($option_value['a'][0]['title'])) > 0){
    	                    $OPTION['values'][] = array('name' => trim($option_value['a'][0]['title']) , 'price' => 0);
    	                }
    	            }
