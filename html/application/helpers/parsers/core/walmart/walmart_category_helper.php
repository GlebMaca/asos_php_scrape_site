...
function parse_products($html , $task){
        ...
        $link = 'ul.search-result-gridview-items li a';
        $parser = new nokogiri($html);
        $links = $parser->get($link)->toArray();
        //echo '<pre>'.print_r($links , 1).'</pre>';exit;
        unset($parser);
        if(isset($links) && is_array($links) && count($links) > 0){
            foreach($links as $link){
                if(isset($link['href']) && stripos($link['href'] , '#customer-reviews') < 1){
                    if(stripos($link['href'] , "almart.com") < 1){
                        $link['href'] = $base . $link['href'];
                    }
                    $result[] = $link['href'];
        ...
        $link = 'ul.search-result-listview-items li a';
        $parser = new nokogiri($html);
        $links = $parser->get($link)->toArray();
        //echo '<pre>'.print_r($links , 1).'</pre>';exit;
        unset($parser);
        ...
        $link = 'div.search-result-gridview-items a';
        $parser = new nokogiri($html);
        $links = $parser->get($link)->toArray();
        //echo '<pre>'.print_r($links , 1).'</pre>';exit;
        unset($parser);
        ...
        $link = 'div.search-result-listview-items a';
        $parser = new nokogiri($html);
        $links = $parser->get($link)->toArray();
        // echo '<pre>'.print_r($links , 1).'</pre>';exit;
        unset($parser);
        ...
        // more URLs
        $tres = explode('","productPageUrl":"' , $html);
        if(count($tres) > 1){
            unset($tres[0]);
            foreach($tres as $bl){
                $ttres = explode('"', $bl, 2);
                $link = $ttres[0];
                if(stripos($link , "almart.com") < 1){
                    $link = $base . $link;
                }
                $result[] = $link;
            }
...
function parse_next_page($html , $task){
        ...
        $tres = explode('"next":{"url":"' , $html);
        if(count($tres) > 1){
           $ttres = explode('"', $tres[1] , 2);
           $out = 'https://www.walmart.com/search/?' . $ttres[0];
           // echo $out;exit;
           return $out;
        }
