...
function mspro_rosegal_description($html){
	...
	$temp  = $pq->find('div.detail_editable');
	foreach ($temp as $block){
	    $res .= '<div>' . $temp->html() . '</div>';
	}
	
	$temp  = $pq->find('div.section-goods-desc');
	foreach ($temp as $block){
	    $res .= '<div>' . $temp->html() . '</div>';
	}
...
function mspro_rosegal_price($html){
    ...
    $res =  explode('roperty="product:price:amount" content="' , $html);
    if(count($res) > 1){
        $res = explode('"' , $res[1]);
        if(count($res) > 1){
            $price = preg_replace("/[^0-9.]/", "",  trim($res[0]) );
    ...
    $res =  explode('o.goods.price = "' , $html);
    if(count($res) > 1){
        $res = explode('"' , $res[1]);
        if(count($res) > 1){
            $price = preg_replace("/[^0-9.]/", "",  trim($res[0]) );
