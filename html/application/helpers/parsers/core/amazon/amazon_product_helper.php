...
function mspro_amazon_title($html){ 
    $res = explode('<h1 class="a-size-large a-spacing-micro">' , $html);
    if(count($res) > 1){
...
function mspro_amazon_description($html , $url){
       ...
       if(count($t_res) > 1){
		...
		foreach ($temp as $block){
		    // https://www.amazon.com/dp/B07KRYTFQG
			//$res .= utf8_decode($temp->html()).'<br />';
		    $res .= $temp->html().'<br />';
		}
		
		
		$temp  = $pq->find('div#descriptionAndDetails');
		foreach ($temp as $block){
		    // https://www.amazon.com/dp/B07KRYTFQG
		    //$res .= utf8_decode($temp->html()).'<br />';
		    $res .= $temp->html().'<br />';
		...
		/*$t = explode('descriptionAndDetails' , $html);
		echo count($t);//exit;
		echo $res;exit;*/
...
function mspro_amazon_weight($html){
        ...
        if(count($out) < 1){
            $res = explode('>Item Weight</td><td class="value">' , $html);
            if(count($res) > 1){
                $res = explode('<' , $res[1] , 2);
                if(count($res) > 1){
                    $weight = $res[0];
...
function mspro_amazon_dimensions($html){
    $out = array();
    $res = explode('Product Dimensions</td>' , $html);
    if(count($res) > 1){
        $res = explode('</' , $res[1] , 2);
        if(count($res) > 1){
            $dims = strip_tags($res[0]);
            $t_res = explode(";" , $dims);
...
function mspro_amazon_isbn($html){
    $res =  explode('id="ASIN" name="ASIN" value="' , $html);
    if(count($res) > 1){
        $res = explode('"' , $res[1]);
        if(count($res) > 1){
            return trim($res[0]);
...
function mspro_amazon_other_images($html , $url , $color_images = false){
 ...
 function mspro_amazon_options($html){
            ...
            if(count($res) > 1){
                ...
                if(count($sizes) > 1){
                    ...
                    foreach($sizes as $size){
                        $OPTION['values'][] = array('name' => $size , 'price' => 0);
                    }
                    if(count($OPTION['values']) > 0){
                        $out[] = $OPTION;
    ...
    if(!$colors_option_exists){
        $instruction = 'ul.imageSwatches li img';
        $parser = new nokogiri($html);
        $res = $parser->get($instruction)->toArray();
        // echo '<pre>'.print_r($res , 1).'</pre>'; //exit;
        unset($parser);
        if(isset($res) && is_array($res) && count($res) > 0){
            ...
            $OPTION['values'] = array();
            foreach($res as $option_value){
                if(isset($option_value['alt']) && !is_array($option_value['alt']) && strlen(trim($option_value['alt'])) > 0 ){
                    $ARR = array('name' => trim($option_value['alt']) , 'price' => 0);
                    if(isset($option_value['src']) && !is_array($option_value['src']) && strlen(trim($option_value['src'])) > 0 ){
                        $ARR['image'] = $option_value['src'];
                        $originalType = 'image';
                    }
                    $OPTION['values'][] = $ARR;
                }
...
function mspro_amazon_noMoreAvailable($html){
    if(stripos($html , '>Currently unavailable.') > 0 || stripos($html , 'Attualmente non disponibile.') > 0){
        return true;
    }
	return false;
}
