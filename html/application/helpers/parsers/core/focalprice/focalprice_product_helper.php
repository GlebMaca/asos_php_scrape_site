...
function mspro_focalprice_description($html){
	...
	$temp  = $pq->find('section.product-description');
	foreach ($temp as $block){
	    $res .= '<div>' . $temp->html() . '</div>';
	}
...
function mspro_focalprice_price($html){
    $res = explode('<span id="our_price_display" class="price" itemprop="price" content="' , $html);
    if(count($res) > 1){
        $res = explode('"' , $res[1] , 2);
        if(count($res) > 1){
    ...
    $res = explode('" id="unit_price">' , $html);
    if(count($res) > 1){
...
function mspro_focalprice_priceOriginal($html){
    $res = explode('" id="market_price">' , $html);
    if(count($res) > 1){
        $res = explode('</' , $res[1] , 2);
        if(count($res) > 1){
...
function mspro_focalprice_sku($html){
    $res =  explode('itemprop="sku" content="' , $html);
    if(count($res) > 1){
        $res = explode('"' , $res[1]);
        if(count($res) > 1){
            return str_replace(array("&nbsp;" , "&amp;") , array(" " , "`") , $res[0]);
    ...
    $res =  explode("var productReference='" , $html);
    if(count($res) > 1){
        $res = explode("'" , $res[1]);
        if(count($res) > 1){
            return str_replace(array("&nbsp;" , "&amp;") , array(" " , "`") , $res[0]);
...
function mspro_focalprice_main_image($html){
    $arr = focalprice_get_images($html);
...
function focalprice_get_images($html){
		$out = array();
		
		$instruction = 'div#thumbs_list ul li img';
		$parser = new nokogiri($html);
		$data = $parser->get($instruction)->toArray();
		...
		if(isset($data) && is_array($data) && count($data) > 0){
		    foreach($data as $pos_image){
		        if(isset($pos_image['src']) && !is_array($pos_image['src'])){
		    			     $out[] = str_ireplace("-small_default" , "" , $pos_image['src']);
		        }
		    }
		}
...
function mspro_focalprice_options($html){
	$out = array();
	    ...
	    $curl = curl_init();
	    curl_setopt($curl, CURLOPT_URL, 'https://www.focalprice.com/ProductDetail/GetProductGroup');
	...
	if(count($out) < 1){
		$res = getUrl('https://www.focalprice.com/ProductDetail/GetProductGroup' , array('sku' => mspro_focalprice_sku($html) ));
