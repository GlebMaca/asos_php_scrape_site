...
function mspro_souq_price($html){
        ...
        $res = explode('","product_price":' , $html);
        if(count($res) > 1){
            $res = explode(',' , $res[1] , 2);
            if(count($res) > 1){
                $res = preg_replace("/[^0-9.]/", "",  $res[0]);
...
function mspro_souq_sku($html){
    ...
    $res = explode('<dt>Item EAN</dt><dd>' , $html);
    if(count($res) > 1){
        $res = explode('</' , $res[1] , 2);
        if(count($res) > 1){
            return trim($res[0]);
    ...
    if(count($res) > 1){
		...
		return mspro_souq_ean($html); 
}

function mspro_souq_model($html){
    $res = explode('<dt>Model Number</dt><dd>' , $html);
    if(count($res) > 1){
        $res = explode('</' , $res[1] , 2);
        if(count($res) > 1){
            return trim($res[0]);
...
function mspro_souq_options($html){
    ...
    $instruction = 'div.product-connections ul';
    $parser = new nokogiri($html);
    $res = $parser->get($instruction)->toArray();
    //echo '<pre>'.print_r($res , 1).'</pre>';exit;
    if(isset($res) && is_array($res) && count($res) > 0){
        foreach($res as $pos_option){
            if(isset($pos_option['id']) && !is_array($pos_option['id']) && isset($pos_option['data-type']) && !is_array($pos_option['data-type']) && isset($pos_option['li']) && count($pos_option['li']) > 0){
                $OPTION = array();
                $OPTION['name'] = trim($pos_option['data-type']);
                $OPTION['type'] = "select";
                $OPTION['required'] = true;
                $OPTION['values'] = array();
                // try to get INNITIAL VALUE
                $instruction = 'div[data-toggle=' . $pos_option['id'] . '] span';
                $parser = new nokogiri($html);
                $res = $parser->get($instruction)->toArray();
                //echo '<pre>'.print_r($res , 1).'</pre>';exit;
                if(isset($res[0]['#text']) && !is_array($res[0]['#text'])){
                    $OPTION['values'][] = array('name' => $res[0]['#text'] , 'price' => 0);
                }
                foreach($pos_option['li'] as $option_value){
                    if(isset($option_value['data-value']) && !is_array($option_value['data-value'])){
                        $OPTION['values'][] = array('name' => $option_value['data-value'] , 'price' => 0);
                    }elseif(isset($option_value['a'][0]['#text']) && !is_array($option_value['a'][0]['#text'])){
                        $OPTION['values'][] = array('name' => $option_value['a'][0]['#text'] , 'price' => 0);
                    }
                }
    ...
    /*$instruction = 'div.product-connections';
    $parser = new nokogiri($html);
    $res = $parser->get($instruction)->toArray();
    echo '<pre>'.print_r($res , 1).'</pre>';exit;*/
