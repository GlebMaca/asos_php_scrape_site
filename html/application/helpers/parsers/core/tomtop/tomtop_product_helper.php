...
function mspro_tomtop_options($html){
    ...
    if(isset($res) && is_array($res) && count($res) > 0){
        ...
        foreach($res as $option_value){
            if(isset($option_value['data-attr-value']) && !is_array($option_value['data-attr-value'])){
                $OPTION['values'][] = array('name' =>trim(ucfirst($option_value['data-attr-value'])) , 'price' => 0);
            }
        }
        if(count($OPTION['values']) > 0){
            $out[] = $OPTION;
