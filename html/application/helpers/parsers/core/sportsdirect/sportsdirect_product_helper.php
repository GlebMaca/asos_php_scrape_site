...
function mspro_sportsdirect_title($html){
        $res =  explode('<!-- mp_trans_rt_start id="1" args="as" 5 -->' , $html);
        if(count($res) > 1){
            $res = explode("<!--" , $res[1] , 2);
            if(count($res) > 1){
                return trim($res[0]);
...
function mspro_sportsdirect_options($html){
    $out = array();
    $initialPrice = mspro_sportsdirect_price($html);
    //echo $html;
	...
	if(count($res) > 1){
	    ...
	    if(count($res) > 1){
	        ...
	        if($res && isset($res[0]['SizeVariants']) && count($res[0]['SizeVariants']) > 0){
	            ...
	            $OPTION['required'] = true;
	            $OPTION['values'] = array();
	            foreach($res[0]['SizeVariants'] as $option_value){
	                if(isset($option_value['SizeName']) && !is_array($option_value['data-text'])){
	                    $price = 0;
	                    if(isset($option_value['ProdSizePrices']['SellPrice']) && !is_array($option_value['ProdSizePrices']['SellPrice'])){
	                        $price = mspro_sportsdirect_get_price_diff($option_value['ProdSizePrices']['SellPrice'] , $initialPrice);
	                    }
	                    $OPTION['values'][] = array('name' => trim($option_value['SizeName']) , 'price' => $price);
	                } 
	            }
	            if(count($OPTION['values']) > 0){
	                $out[] = $OPTION;
...
function mspro_sportsdirect_get_price_diff($optionPrice , $initialPrice){
    $optionPrice = str_ireplace(array(","), array("."), $optionPrice);
    $optionPrice = preg_replace("/[^0-9.]/", "", $optionPrice);
    $res = (float) ((float) $optionPrice - (float) $initialPrice);
    return $res;
}
...
function mspro_sportsdirect_noMoreAvailable($html){
    if(stripos($html , 'data-preorder="Pre-order now">Pre-order now') > 0){
        return true;
    }
	return false;
}
